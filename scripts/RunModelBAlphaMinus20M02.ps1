param(
  ...
  [Parameter(Mandatory=$true)][double]$Phi,
  [string]$Out = ''
)
...
if ([string]::IsNullOrWhiteSpace($Out)) { $Out = ('H:\comsolcc\comsol_magnet\work\modelB_alpha_minus20_M02_z{0}_phi{1}_{2}' -f $Z,$Phi,(Get-Date -Format 'yyyyMMdd_HHmmss')) }
...
$argLine = '-Xmx12g -cp "H:\comsolcc\comsol_magnet\scripts;{0}" RunModelBLocalMeshConvergence "{1}" "{2}" alphaminus20m02 {3} {4}' -f $Plugins,$Source,$Out,$Z,$Phi
...
Write-Output ("PID={0} OUT={1} ALPHA=-20 Z={2} PHI={3}" -f $proc.Id,$Out,$Z,$Phi)
