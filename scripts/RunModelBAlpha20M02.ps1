...
if ([string]::IsNullOrWhiteSpace($Out)) { $Out = ('H:\comsolcc\comsol_magnet\work\modelB_alpha20_M02_z{0}_phi{1}_{2}' -f $Z,$Phi,(Get-Date -Format 'yyyyMMdd_HHmmss')) }
...
$argLine = '-Xmx12g -cp "H:\comsolcc\comsol_magnet\scripts;{0}" RunModelBLocalMeshConvergence "{1}" "{2}" alpha20m02 {3} {4}' -f $Plugins,$Source,$Out,$Z,$Phi
